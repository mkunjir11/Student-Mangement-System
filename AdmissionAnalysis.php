        ...
        <?php
            if($_SERVER["REQUEST_METHOD"]=="POST")  
            {
                ...
                try 
                {
                    ...
                    $cursor = $collection->find(array('yearAd' => $year, 'dept' => $dept, 'classYear' => $class)); 
                    
                                     
                    if (is_array($cursor) || is_object($cursor))
                    { 
                       $map = new MongoCode("function() { emit(this.admCat,1); }");
			...
			"map" => $map,
			"reduce" => $reduce,
                        "query" => array('yearAd' => $year,"dept" => $dept,'classYear' => $class),
			"out" => 'test_mapreduce_capMgmt'));
			
                        $users = $db->selectCollection('test_mapreduce_capMgmt')->find();                      
                     ?>
			 <!-- load api -->
                        ...
                        <script type="text/javascript">
                            function drawVisualization() 
                            {
                                ...
                                new google.visualization.PieChart(document.getElementById('visualization')).
                                        draw(data, {title:"CAP / Management Ratio of class <?PHP echo $class; ?> in Department <?PHP echo $dept;  ?>"});
                            }
                      ...
                      <h2>Grid View</h2>
                        ...
                        <Label style="color: red;font-size: medium" ><br><?PHP if($_SERVER["REQUEST_METHOD"]=="POST"){echo "Filters set :: Year: ".$year." Department: ".$dept. " Class: ".$class;} ?></label>
