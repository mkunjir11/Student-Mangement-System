        ...
        <?php
            if($_SERVER["REQUEST_METHOD"]=="POST")  
            {
                ...
                try 
                {
                    ...
                    { 
                        $map = new MongoCode("function() { emit(this.classYear,1); }");
			...
			"map" => $map,
			"reduce" => $reduce,
                         "query" => array("yearAd" => $year, "dept" => $dept ),
			"out" => 'test_mapreduce_acdYearIntake'));
			
                        $users = $db->selectCollection('test_mapreduce_acdYearIntake')->find();
                        $collection2 = $db->test_mapreduce_acdYearIntake;
			 ...
			 <!-- load api -->
                        ...
                        <script type="text/javascript">
                            function drawVisualization() 
                            {
                                ...
                                new google.visualization.PieChart(document.getElementById('visualization')).
                                        draw(data, {title:"Total students in every class of Department <?PHP echo $dept;?>"});
                            }
                      ...
                      <h2>Grid View</h2>
                        ...
                        <Label style="color: red;font-size: medium" ><br><?PHP if($_SERVER["REQUEST_METHOD"]=="POST"){echo "Filters set :: Year: ".$year." Department: ".$dept;} ?></label>
